...
namespace Kirurobo
{
    ...
    internal class UniWinCore : IDisposable
    {
        ...
        public UniWinCore()
        {
            ...
            {
                Debug.LogError($"LibUniWinC.dll が見つかりません。Plugins/Windows/ フォルダに適切なDLLファイルが配置されているか確認してください。\nエラー詳細: {ex.Message}");
                throw;
            }
            catch (Exception ex)
            {
                Debug.LogError($"LibUniWinC.dll の読み込み中にエラーが発生しました: {ex.Message}");
                throw;
            }
        }
        ...
        public bool AttachMyWindow()
        {
            ...
            {
                Debug.LogError($"LibUniWinC.dll が見つかりません: {ex.Message}");
                IsActive = false;
                return false;
            }
            catch (Exception ex)
            {
                Debug.LogError($"ウィンドウアタッチ中にエラーが発生しました: {ex.Message}");
                IsActive = false;
                return false;
            }
        }
